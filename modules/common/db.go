...
func (d *db) create(room *TopicRoom) error {
	...
	if _, err = d.addMemberTx(tx, room.RoomID, room.ChannelID, room.CreatorUID, room.CreatorName, room.CreatorAvatar, 1); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (d *db) addMember(roomID, channelID, uid, name, avatar string) error {
	...
	inserted, err := d.addMemberTx(tx, room.RoomID, room.ChannelID, uid, name, avatar, role)
	if err != nil {
		return err
	}
	if inserted {
		users := dedupReplyUsers("", append([]ReplyAvatar{{UID: uid, Name: name, Avatar: avatar}}, room.ReplyUsers...), DefaultMaxReplyAvatars)
		usersJSON, _ := json.Marshal(users)
		if _, err = tx.Update("topic_rooms").
			Set("participant_count", dbr.Expr("participant_count+1")).
			Set("reply_users_json", string(usersJSON)).
			Set("updated_at", dbr.Expr("NOW()")).
			Where("status=1 AND room_id=?", room.RoomID).Exec(); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (d *db) addMemberTx(tx *dbr.Tx, roomID, channelID, uid, name, avatar string, role int) (bool, error) {
...
func (d *db) get(roomID string) (*TopicRoom, error) {
	...
	normalizeParticipantCount(rooms[0])
	return rooms[0], nil
}
...
func normalizeParticipantCount(r *TopicRoom) {
	if r == nil {
		return
	}
