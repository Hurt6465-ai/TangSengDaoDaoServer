...
func profileSelect(profileAlias, userAlias string) string {
	return fmt.Sprintf(`%[1]s.uid,
        ...
        %[1]s.enabled,%[1]s.user_paused,%[1]s.intent,%[1]s.cross_border_preference,%[1]s.gender_preference,%[1]s.min_age,%[1]s.max_age,%[1]s.city,
...
func (d *db) saveProfile(uid string, req SaveProfileReq) (*DatingProfileResp, error) {
	...
	// 新版本只保留一套图片；card_photos 仅作为旧客户端兼容镜像。
	cardPhotosRaw := photosRaw
...
func (d *db) setEnabled(uid string, enabled int, userPaused int) (*DatingProfileResp, error) {
	if enabled != 1 {
		enabled = 0
	}
	if userPaused != 1 {
	...
	builder := d.session.Update("dating_profiles").Set("enabled", enabled).Set("user_paused", userPaused).
		Set("last_active_at", time.Now().UnixMilli()).Set("updated_at", dbr.Expr("NOW()")).Where("uid=?", uid)
	if enabled == 1 {
...
func (d *db) upsertLocation(uid string, req LocationReq) (*DatingProfileResp, error) {
	...
	refreshBefore := now - DatingLocationRefreshIntervalMS
	...
	// 第一次定位、已过期或距离上次成功更新满72小时才写库。
	_, err := update.Where(`uid=? AND (location_updated_at<=0 OR location_updated_at<=? OR location_updated_at>? OR expires_at<=?)`,
		uid, refreshBefore, now+int64(5*time.Minute/time.Millisecond), now).Exec()
	if err != nil {
		return nil, err
	}
	return d.getProfile(uid)
...
func (d *db) markServed(uid, sessionID string, profiles []*DatingProfileResp) error {
	...
	args := make([]interface{}, 0, len(profiles)*5)
	seen := make(map[string]struct{}, len(profiles))
	for _, profile := range profiles {
		if profile == nil || strings.TrimSpace(profile.UID) == "" {
			continue
		}
		if _, ok := seen[profile.UID]; ok {
			continue
		}
		seen[profile.UID] = struct{}{}
		values = append(values, "(?,?,?,?,?,NOW())")
		args = append(args, uid, sessionID, profile.UID, now, now+DatingServedTTLMS)
	}
	if len(values) == 0 {
		return nil
	}
	_, err := d.session.InsertBySql(`INSERT INTO dating_served(uid,session_id,to_uid,served_at,expires_at,created_at) VALUES `+
		strings.Join(values, ",")+` ON DUPLICATE KEY UPDATE served_at=VALUES(served_at),expires_at=VALUES(expires_at)`, args...).Exec()
	return err
}
...
func (d *db) createMatch(uid, toUID string) (*datingMatchModel, bool, error) {
	...
	result, err := d.session.InsertBySql(`INSERT INTO dating_matches(match_id,pair_key,uid_a,uid_b,status,notice_sent,friend_auto_created,friend_synced,matched_at,created_at,updated_at)
        VALUES(?,?,?,?,1,0,0,0,?,NOW(),NOW())
        ON DUPLICATE KEY UPDATE
          notice_sent=IF(status=2,0,notice_sent),
          friend_auto_created=IF(status=2,0,friend_auto_created),
          friend_synced=IF(status=2,0,friend_synced),
...
func (d *db) claimMatchNotice(matchID string) (bool, error) {
	result := d.session.Update("dating_matches").
		Set("notice_sent", 2).
		Set("updated_at", dbr.Expr("NOW()"))
	result = result.Where(`match_id=? AND status=? AND
		(notice_sent=0 OR (notice_sent=2 AND updated_at<DATE_SUB(NOW(),INTERVAL 2 MINUTE)))`, matchID, DatingMatchActive)
	execResult, err := result.Exec()
	if err != nil {
		return false, err
	}
...
func (d *db) markMatchNoticeSent(matchID string) error {
	builder := d.session.Update("dating_matches").Set("notice_sent", 1).Set("updated_at", dbr.Expr("NOW()"))
	_, err := builder.Where("match_id=? AND status=? AND notice_sent=2", matchID, DatingMatchActive).Exec()
	return err
}

func (d *db) releaseMatchNoticeClaim(matchID string) {
	builder := d.session.Update("dating_matches").Set("notice_sent", 0).Set("updated_at", dbr.Expr("NOW()"))
	_, _ = builder.Where("match_id=? AND status=? AND notice_sent=2", matchID, DatingMatchActive).Exec()
}

func (d *db) markMatchFriendSynced(matchID string, autoCreated bool) (bool, error) {
	result, err := d.session.Update("dating_matches").
		Set("friend_auto_created", boolInt(autoCreated)).
		Set("friend_synced", 1).
		Set("updated_at", dbr.Expr("NOW()")).
		Where("match_id=? AND status=?", matchID, DatingMatchActive).Exec()
	if err != nil {
		return false, err
	}
...
func (d *db) getMatchByID(matchID string) (*datingMatchModel, error) {
	...
	_, err := d.session.Select("match_id", "uid_a", "uid_b", "status", "notice_sent", "friend_auto_created", "friend_synced").From("dating_matches").Where("match_id=?", matchID).Load(&model)
	return model, err
}

func (d *db) getMatchByPair(uid, toUID string) (*datingMatchModel, error) {
	var model *datingMatchModel
	_, err := d.session.Select("match_id", "uid_a", "uid_b", "status", "notice_sent", "friend_auto_created", "friend_synced").From("dating_matches").Where("pair_key=?", pairKey(uid, toUID)).Load(&model)
	return model, err
}
...
func (d *db) matches(uid string, limit int) ([]*DatingMatchResp, error) {
	...
	var rows []*matchRow
	_, err := d.session.SelectBySql(`SELECT match_id,status,notice_sent,friend_auto_created,friend_synced,IF(uid_a=?,uid_b,uid_a) AS other_uid,
	...
	for _, item := range rows {
		...
		out = append(out, &DatingMatchResp{
			MatchID: item.MatchID, Status: item.Status, CreatedAt: item.CreatedAt, UpdatedAt: item.UpdatedAt,
			NoticeSent: item.NoticeSent, FriendAutoCreated: item.FriendAutoCreated, FriendSynced: item.FriendSynced, User: profile,
		})
	}
	return out, nil
}
...
func (d *db) cancelMatch(uid, matchID string) error {
	// 屏蔽状态优先级高于取消匹配，不能被取消接口降级成可重新匹配状态。
	_, err := d.session.Update("dating_matches").Set("status", DatingMatchCanceled).Set("updated_at", dbr.Expr("NOW()")).
		Where("match_id=? AND (uid_a=? OR uid_b=?) AND status<>?", matchID, uid, uid, DatingMatchBlocked).Exec()
	return err
}
