...
const (
	...
	DatingLocationTTLMS      = int64(30 * 24 * time.Hour / time.Millisecond)
...
func (p *DatingProfileResp) Normalize() {
	...
	p.CardPhotos = alignCardPhotos(p.Photos, parseImageList(p.CardPhotosRaw, DatingMaxPhotos))
...
func buildPhotoCards(p *DatingProfileResp) []DatingPhotoCard {
	...
	if p.DistanceMeters > 0 && p.ShowDistance == 1 {
		if location != "" {
			location += " · " + formatDistance(p.DistanceMeters)
		} else {
			location = formatDistance(p.DistanceMeters)
		}
	}
...
func formatDistance(m int) string {
	...
	return fmt.Sprintf("%.1fkm", float64(m)/1000.0)
}
